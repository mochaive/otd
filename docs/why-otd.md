...
GTD asks: what should I do from this list right now?
OTD asks: what am I committing to finish today?
...
OTD is not a replacement for discipline or judgment. You still decide what matters. You still steer your own life: daily by curating what surfaced, weekly by reviewing what's active, quarterly by setting direction. What the framework does is remove the structural friction that makes that steering harder than it needs to be, and let you spend agency where it pays off, not on a daily ceremony you can't sustain.

Different people fail in different ways. The over-deliberator burns out on a queue someone else built. The over-wanter freezes in front of too many morning choices. The drifter executes for a year and looks up to find no direction. And the newest profile (the approver) delegates faster than they can judge, so the decisions still happen on paper and no longer in fact. Most productivity systems address only the first failure and quietly punish the rest. OTD is built so all four profiles can run it sustainably. It does that by letting the cadence of agency vary, not the requirement of agency itself. If you want to know which profile you fall into, see [Which failure mode are you prone to?](/principles#which-failure-mode-are-you-prone-to)
