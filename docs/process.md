...
Inbox item
  │
  ├─ Actionable? No ──→ Delete / Reference / Someday / Wishlist
  │
  └─ Actionable? Yes
       │
       ├─ Who executes? Me          →  Do       → Place in Area / Flow
       ├─ Who executes? Someone else →  Delegate → Place in Area / Flow
       └─ Who executes? Waiting      →  Await    → Place in Area / Flow
```
...
- **Move to Someday.** Deferred commitment: you intend to act on this and can say what step 1 would be.
- **Move to Wishlist.** Uncommitted possibility: interesting idea, but not a plan and no obligation to pursue. See [Today, Someday & Wishlist](/today-someday-wishlist#wishlist) for the discriminator.
...
- **Never put an item back into Inbox.** Make a decision: trash, Reference, Someday, Wishlist, or Action. "I'll deal with this later" is not a decision. It's a delay. Later, re-captured in the same Inbox, with the same lack of context.
