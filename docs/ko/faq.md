...
**Project와 Flow의 차이는 뭔가요?**

Project는 여러 Flow를 포함하는 큰 목표다. Flow는 하나의 결과를 향한 구체적인 순차 작업 흐름이다.

"이직 준비"는 포트폴리오 준비, 기업 컨택, 인터뷰 준비라는 여러 Flow를 포함한다면 Project다. 각각이 Flow다. 작업 흐름이 하나뿐이라면 Project를 건너뛰고 Flow를 Area 바로 아래에 놓는다.
...
**4계층(Area → Project → Flow → Action)이 너무 복잡하지 않나요?**

실제로는 대부분의 태스크가 2~3계층만 쓴다. 어떤 흐름에도 속하지 않는 독립 태스크는 Area 바로 아래에 있다. 단순한 작업 흐름은 Area → Flow → Action이면 충분하다. Project는 여러 Flow가 공통 목표를 가질 때만 등장한다. 모든 태스크에 4계층을 전부 쓰지는 않는다.
