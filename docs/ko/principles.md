...
OTD는 네 가지 원칙 위에 세워져 있다. 프레임워크의 나머지 모든 것은 이 네 원칙에서 나온다.
...
신뢰할 수 있는 외부 시스템은 이 두 문제를 함께 해결한다. 잊지 않게 된다. 잊을까봐 걱정하지 않게 된다. "이걸 기억해야 한다는 걸 기억해야 해"라는 주의력 세금이 사라진다.
...
### 생산성 시스템이 막아야 할 네 가지 실패 모드

이들은 서로 반대 방향으로 당긴다. 그래서 단일 룰로는 다 못 잡는다:
...
- **승인 형식화 실패**: 결정 지점은 있는데 결정이 사라진다. 돌아오는 일이 판단하는 속도보다 빨라서, 승인이 반사 행동이 된다. 형식상으로는 네가 통제하고 실질적으로는 아니다. 장기 비용: 주체성의 착각. 표류를 가려주기 때문에 없는 것보다 나쁘다. 진단: 승인율이 100%에 붙어 있다.
...
- AI에 자주 위임하고, 결과가 읽는 속도보다 빨리 돌아온다
- 돌아온 것을 마지막으로 거절한 게 언제인지 기억나지 않는다
- 판단하기 위해서가 아니라 큐를 비우기 위해 묶어서 승인한다
- 어제 시스템에서 무엇이 바뀌었는지 알려면 로그를 봐야 한다

이 프로필은 새로 생긴 것이다. 위임이 싸지기 전에는 거의 존재하지 않았다. 앞의 세 가지는 *무엇을 할지 고르는* 문제이고, 이것은 *돌아온 것을 받아들이는* 문제다. **권장 cadence:** 판단이 다시 판단처럼 느껴질 때까지 Review 예산을 낮추고, 쓰기 권한을 가진 agent 수를 줄이고, 기존 작업을 바꾸거나 지우는 반환에는 마찰을 걸어라. 답은 더 빨리 승인하는 것이 아니라 하루에 더 적게 승인하는 것이다.
...
OTD에서 AI는 실행자다. Today를 채우고, swap을 제안하고, Stuck Signal을 surface하고, Delegate / Await / Review 사이클을 처리할 수 있다. 할 수 없는 건 이런 결정이다: 너의 테마, Goal, 새로 활성화할 때 닫히는 것.

AI에서 돌아온 것은 Review를 지나서 들어온다. 곧바로 들어오지 않는다(원칙 4). 하루를 채우는 일은 위임할 수 있다. 그 결과를 받아들이는 일은 위임할 수 없다.
...
제한하는 것은 **활성 집합의 크기**다. 5~10개 Flow가 병렬로 굴러가는 것은 정상이다. 원칙은 가장자리에서, 한 개를 더 추가하려는 순간 활성화된다. 추가하기 전에, 무엇이 떠나는지 결정해라.
...
원칙이 하는 일은 그 순간을 명명하는 것이다. 새 Flow를 만들 때, "무엇이 닫히는가?"라는 질문이 사후 고려가 아니라 활성화의 일부가 된다.

---

## 4. 승인 없이 들어오는 것은 없다.

Await에서 돌아온 일은 Review를 지나 들어온다. 도착하는 순간 반영되지 않는다.

### 왜 위임에는 반환 게이트가 필요한가

원칙 1부터 3은 입력, 주체성, 활성 집합의 천장을 다룬다. 이 셋은 한 가지를 가정한다: 위임한 것이 보낸 대로, 내가 소화할 수 있는 속도로 돌아온다는 것.

사람에게 위임할 때는 대체로 맞는 가정이다. 동료가 답장하고, 나는 그 답장을 읽는다. 위임이 느리고 양이 적으니 반환은 알아서 처리된다.

AI에게 위임할 때는 맞지 않는다. agent 하나가 몇 분 안에 열 건을 돌려보낼 수 있고, 대부분의 도구는 그 결과를 도착하는 즉시 반영한다. 일이 나를 거치지 않고 시스템에 들어온다.

그러면 원칙 1이 반대편에서 깨진다. 여전히 모든 것이 시스템에 있지만, 그중 무엇을 내가 선택했는지 알 수 없게 된다. 자기 결정을 구분할 수 없는 시스템은 신뢰할 수 있는 시스템이 아니다.

### 게이트

- **Await → Review → 안으로.** 돌아온 일에 다른 경로는 없다.
- **Review는 결정이다:** 받아들이거나, 거절하거나, 다시 보낸다.
- **읽기에는 게이트가 필요 없다.** 시스템을 바꾸는 것에만 필요하다.
- **거절도 흔적을 남긴다.** 조용히 사라지는 것도, 조용히 들어오는 것도 없다.

### actor가 중요하다

Await와 Review는 actor를 함께 갖는다: **사람, agent, 규칙**([Mode](/ko/mode) 참고). 셋 다 같은 게이트를 지나지만 실패하는 방식이 다르다:

- **사람**: 느리고, 적고, 잊기 쉽다. 위험은 추적을 놓치는 것.
- **agent**: 빠르고, 많고, 그럴듯하다. 위험은 양이 판단을 앞지르는 것.
- **규칙**: 작동하는 동안 보이지 않는다. 위험은 내가 그것을 썼다는 사실을 잊는 것. 규칙은 내가 내 문장으로 다시 읽을 수 있는 동안만 정당하다.

### Review에는 예산이 있다

예산 없는 게이트는 두 번째 Inbox가 된다. 원칙 1이 막으려던 바로 그 실패가 뒷문으로 들어오는 것이다.

하루에 검토할 양에 상한을 둔다. 상한에 닿으면 **기다리는 것은 기계이고 너가 아니다**: agent는 네가 자리를 만들 때까지 일을 돌려보낼 수 없다. 검토되지 않은 것은 쌓이는 대신 나이를 먹고 Stuck Signal로 올라온다.

### 시스템이 보증할 수 없는 것

Await는 네가 무언가를 넘겼다는 사실을 기록한다. 그 일이 실제로 됐는지는 검증하지 않는다. 돌아온 결과는 **주장**이고, Review는 그 주장을 판단하는 자리다. 이걸 얼버무리는 프레임워크는 위임을 신앙으로 바꾼다.

### 왜 기능이 아니라 원칙인가

도구가 게이트를 구현할 수 있고, 좋은 도구는 그래야 한다. 하지만 승인이 생각보다 싸다면 게이트는 무용하다(원칙 2의 **승인 형식화 실패**를 보라). 이것을 원칙으로 만드는 것은 기준이다: *최종 결정은 사람이 한다. 일이 시스템에 들어오는 그 순간에.* 도구는 문을 잡아줄 수 있다. 무엇이 그 문을 지날지는 너만 정할 수 있다.
...
원칙 2가 주체성을 만든다: 너는 매일은 가벼운 큐레이션으로, 매주는 리뷰로, 분기는 방향 설정으로 활성 삶의 저자로 남는다. cadence는 너의 것이지만, *어딘가에서* 키를 잡고 있다는 사실은 협상 불가다.

원칙 3이 경계를 만든다: 선택지를 고를 활성 집합이 시간이 지나도 살 만한 상태로 유지된다.

원칙 4가 봉인을 만든다: 누가, 무엇이 만들어낸 것이든 너의 결정 없이는 시스템에 들어오지 않는다.

네 원칙이 함께 작동하면 구체적인 결과가 나온다: 지금 뭘 하는지 놀랍지 않고, 반응만 하고 있는 느낌이 없고, 시스템이 끝낼 수 없는 백로그로 조용히 부풀지 않는다. 원칙 1이 모든 걸 담는다. 원칙 2는 매일의 의례 없이도 너를 운전석에 둔다. 원칙 3은 그 도로를 살 만한 상태로 유지한다. 원칙 4는 그 도로를 너의 것으로 유지한다. 위임이 싸질수록, 네가 선택하지 않은 일로 시스템이 채워지는 것을 막는 원칙이 이것이다.
...
- **기준은 매일 아침 스스로 설정한다.** Daily Review에서 어떤 Action을 Today에 넣을지 선택한다. 가능한 모든 것에 비해 측정되는 게 아니다. 오늘 하기로 결정한 것에 비해 측정된다. Today가 비면 스스로 설정한 기준으로 성공한 것이다.

이것이 덜 일하라는 뜻이 아니다. "더 해야 하는데"라는 배경 소음 없이 일하라는 뜻이다. 그 소음은 비싸다. 집중력을 떨어뜨리고, 불안을 만들고, 결국 그 시스템이 대표하는 심판을 피하기 위해 시스템 자체를 포기하게 만든다.

오늘 할 일이 끝났다는 진짜 신호로 하루를 깔끔하게 끝내는 시스템은 사치가 아니다. 몇 주가 아니라 몇 달, 몇 년에 걸쳐 시스템을 지속 가능하게 만드는 것이다.
