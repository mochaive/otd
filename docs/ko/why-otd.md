...
OTD는 규율이나 판단을 대신하지 않는다. 무엇이 중요한지는 여전히 당신이 결정한다. 당신이 자기 삶을 직접 운전한다: 매일은 올라온 것을 큐레이트하면서, 매주는 활성 집합을 리뷰하면서, 분기는 방향을 설정하면서. 프레임워크가 하는 것은 그 운전을 필요 이상으로 어렵게 만드는 구조적 마찰을 제거하고, 매일 지속할 수 없는 의례에 주체성을 다 쏟지 않고, 효과가 큰 자리에 쓸 수 있게 만드는 것이다.

사람마다 실패하는 방식이 다르다. 과도숙고형은 남이 만든 큐를 처리하다 소진된다. 과욕형은 너무 많은 아침 선택지 앞에서 얼어붙는다. 표류형은 1년을 실행하고 고개를 들었더니 방향이 없다. 그리고 가장 새로운 프로필(승인형)은 판단하는 속도보다 빠르게 위임해서, 결정이 서류상으로만 남고 실질에서는 사라진다. 대부분의 생산성 시스템은 첫 번째 실패만 다루고 나머지를 조용히 처벌한다. OTD는 네 프로필 모두가 지속 가능하게 굴릴 수 있도록 설계됐다. 그 방식은 주체성의 *cadence*를 다양하게 두는 것이지, 주체성 자체의 요구를 양보하는 것이 아니다. 자기가 어느 프로필에 속하는지 알고 싶다면 [어떤 실패 모드에 노출되어 있는가?](/ko/principles#너는-어떤-실패-모드에-노출되어-있는가) 참고.
