...
- **Do** — 내가 직접 실행한다.
- **Delegate** — 다른 누군가 또는 무언가에게 넘긴다: 사람, AI agent, 또는 내가 쓴 규칙.
- **Await** — 위임한 작업이나 외부 입력의 결과를 기다린다.
- **Review** — 결과가 돌아왔다. 검토 후 승인하거나, 거절하거나, 재위임한다. 돌아온 일은 여기로만 들어온다([원칙 4](/ko/principles#_4-승인-없이-들어오는-것은-없다)).

Delegate·Await·Review는 **actor** — 상대가 누구인지 — 를 함께 갖는다. [Actor](#actor-상대는-누구인가) 참고.
...
Await Action은 Today에 자동으로 올라오지 않는다 — 별도로 추적된다. 결과가 도착하면 **Review** Action이 된다. 도착은 Review를 만들 뿐, 시스템에 직접 쓰지 않는다([원칙 4](/ko/principles#_4-승인-없이-들어오는-것은-없다)).
...
- **거절** — 결과가 틀렸거나 원하지 않는다. 들어오지 않는다. 거절도 남아 보이므로 조용히 사라지는 것은 없다.
- **재위임** — 결과는 맞지 않지만 일은 여전히 유효하다. 수정된 지시로 새 Delegate Action을 만든다.
...
**Review에는 예산이 있다.** 위임이 싸지면 판단이 따라가는 속도보다 반환이 빨라지고, 승인이 반사 행동으로 퇴화한다([원칙 2](/ko/principles#_2-오늘을-큐레이트하고-삶을-설계하라)의 **승인 형식화 실패** 참고). 하루에 감당할 Review 수에 상한을 둔다. 상한에 닿으면 기다리는 것은 기계다 — agent는 일을 돌려보낼 수 없고, 검토되지 않은 것은 쌓이는 대신 Stuck Signal로 나이를 먹는다.

---

## Actor: 상대는 누구인가

누가 실행하든 Mode는 같은 세 개(Delegate·Await·Review)다. 하지만 세 actor는 실패하는 방식이 달라서, 지금 상대가 누구인지 이름을 붙일 값이 있다.

| Actor | 양·속도 | 실패 방식 | 필요한 것 |
| --- | --- | --- | --- |
| **사람** | 느리고 적다 | 기다리는 중이라는 걸 잊는다 | Weekly Review에서의 후속 확인 |
| **agent** | 빠르고 많고 그럴듯하다 | 양이 판단을 앞지른다 | Review 예산과 단단한 반환 게이트 |
| **규칙** | 작동하는 동안 보이지 않는다 | 내가 썼다는 사실을 잊는다 | 내 문장으로 다시 읽히는 것 |

**규칙**은 나를 대신해 움직이는, 내가 쓴 한 문장이다 — "2주 동안 손대지 않은 것은 Someday로." 설정이 아니라 actor다. 내가 했을 일을 대신 하기 때문이다.

규칙을 정당하게 유지하는 선:

- 규칙은 **새로 들어온 것을 배치**할 수 있다 — 라우팅, 분류, 태깅.
- 규칙은 **이미 있는 것을 바꾸거나 지울 수 없다** — Review를 지나야 한다.
- 규칙이 한 모든 일은 기록되고 되돌릴 수 있으며, 규칙 자체는 읽고 지울 수 있는 한 줄로 남는다.

이 경계가 규칙과 "더 이상 내가 통제하지 못하는 자동화"를 가른다. 규칙을 내 말로 설명할 수 없게 된 순간, 그건 더 이상 내 것이 아니다 — 폐기하라.
...
같은 Mode 안에서의 순서는 사용자가 결정한다. Do 항목 두 개 중 하나를 고르는 건 가벼운 판단 — 우선순위, 에너지, 컨텍스트에 관한 것이다. [Principle 2](/ko/principles#_2-오늘을-큐레이트하고-삶을-설계하라)가 가장 가볍게 작동하는 자리: 작은 큐레이션 순간이지, 매일의 재설계가 아니다. 그리고 Mode가 그 결정을 더 작게 만든다. 구분 없는 10개 중에서 고르는 대신, 같은 Mode의 2~3개 중에서 고른다.
