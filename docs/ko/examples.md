...
*"Figma 심화 강좌"* → 흥미롭지만 진짜 계획은 없다. 1단계가 뭔지 말할 수 없다: "강좌 찾기"? "어떤 걸로 할지 정하기"? commitment 아닌 한가한 관심. Wishlist.
...
*"Shape Up 읽기"* → Wishlist. 흥미는 있지만 commitment하지 않는다. 1년이 지나도 노화 압박이 없다.

*"경비 정산"* → 행동 가능. Do. "서울 출장 경비 정산 (영수증 사진 앱에 있음)".

→ `Area: 재정 → Action (독립)`

*"개인 사이트 업데이트"* → Someday. 하고 싶지만 지금은 아님. 1단계가 명확하고 ("뭐가 낡았는지 정리"), Q3 덱이 끝나면 활성화할 것. 트리거가 진짜다.
...
Someday: 개인 사이트 업데이트 (트리거: Q3 덱 이후)
Wishlist: Figma 강좌, Shape Up
```
...
배칭할 수 있는 것부터 시작한다. 빠른 관리 항목 두 개(예약하기, 경비 정산)는 합쳐서 15분이면 끝난다. 그 다음 세 개의 프로젝트 Action에 집중 작업.
...
**프로필:** 엔지니어링 매니저. 업무의 절반이 조율, 위임, 막힌 것 해제다. 직접 실행이 아니다.
